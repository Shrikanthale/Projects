...
class Home extends Component {
    constructor(props) {
      ...
      getPosts() {
        //("/posts")
        axios.get("http://localhost:8008/grocery").then(res => { //backend request use
          if (res.data.success) {
            this.setState({
      ...
      handleTextSearch=(event)=>{
        ...
        axios.get("http://localhost:8008/grocery").then(res => { 
          if (res.data.success) {
      ...
      onDelete=(id)=>{
        // console.log(id);
        axios.delete(`http://localhost:8008/grocery/delete/${id}`).then((res)=>{
